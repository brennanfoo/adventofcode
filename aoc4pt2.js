...
pid:745302991 byr:1969 hcl:#7394c7`.replace(/ /g, "\n").split("\n");
...
entries[passport] = new Map();
let specificPassport = entries[passport];
do {
    if (input[i].length === 0) {  // If entry has nothing, create new passport
        passport++;
        console.log("New Passport. Passport #"+passport);
        entries[passport] = new Map();
        specificPassport = entries[passport];
        i++; // Go to next row if line has nothing
    }
    row = input[i].split(":"); // Find key and value
    left = row[0];
    right = row[1];

    if (left !== 'cid'){
        specificPassport.set(left, right); // Assign keys and values to passport

        // Valid passport logic
        if (specificPassport.size === 7){

            // CONSOLE LOGGING TEST
            console.log("Passport #"+passport);
            console.log(valueIsWithin(specificPassport.get('byr'),1920,2002));
            console.log(valueIsWithin(specificPassport.get('iyr'),2010,2020));
            console.log(valueIsWithin(specificPassport.get('eyr'),2020,2030));
            console.log(hgtIsValid(specificPassport.get('hgt')));
            console.log(hclIsValid(specificPassport.get('hcl')));
            console.log(eclIsValid(specificPassport.get('ecl')));
            console.log(pidIsValid(specificPassport.get('pid')));
            console.log(specificPassport);

            if(valueIsWithin(specificPassport.get('byr'),1920,2002) &&
                valueIsWithin(specificPassport.get('iyr'),2010,2020) &&
                valueIsWithin(specificPassport.get('eyr'),2020,2030) &&
                hgtIsValid(specificPassport.get('hgt')) &&
                hclIsValid(specificPassport.get('hcl')) &&
                eclIsValid(specificPassport.get('ecl')) &&
                pidIsValid(specificPassport.get('pid'))) {
                valid++;
                console.log("7 values excluding cid, so valid # now:" + valid);
            }
        } // Or 7 fields and no cid
    }
...
function valueIsWithin(value,min,max){
...
function hgtIsValid(val) {
    ...
    if (str.slice(-2)=="cm"){
        ...
        return valueIsWithin(parseInt(str),150,193);
    } else if (str.slice(-2)=="in"){
        str = str.substring(0, val.length - 1);
        return valueIsWithin(parseInt(str), 59, 76);
    } else {return false;}
}
