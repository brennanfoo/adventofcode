...
do {
    ...
    if (left !== 'cid'){
        ...
        entries[passport].set(left, right); // Assign keys and values to passport

        // Valid passport logic
        if (entries[passport].size === 7 &&
            entries[passport].get('byr') >= 1920 &&
            entries[passport].get('byr') <= 2002 &&
            entries[passport].get('iyr') >= 2010 &&
            entries[passport].get('iyr') <= 2020 &&
            entries[passport].get('eyr') >= 2020 &&
            entries[passport].get('eyr') <= 2030){
            valid++;
            console.log("7 values excluding cid, so valid # now:"+valid+" VALIDVALIDVALIDVALIDVALIDVALIDVALIDVALIDVALID");
        } // Or 7 fields and no cid
    }

    console.log(entries[passport]);
